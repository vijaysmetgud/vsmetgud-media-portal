...
export class SurroundEngine {
  ...
  constructor(el, options = {}) {
    if (!el) {
      throw new Error('SurroundEngine needs a <video> or <audio> element');
    }
    ...
    // FIX:
    // ctx can exist before the entire Web Audio graph has been constructed.
    // Keep graph readiness separate from AudioContext existence.
  ...
  setPreset(name) {
    ...
    // _setActive(true) builds the renderer, but keep the readiness guard
    // in _apply() as a second layer of protection.
    this._apply();
  ...
  setParam(name, value) {
    ...
    // FIX:
    // A context existing does NOT guarantee that shelfL, shelfR, etc.
    // have already been created. Only apply once the graph is ready
    // and the effect is active.
    if (this._graphReady && this._active) {
      this._apply();
    }
  ...
  setOutputMode(mode) {
    ...
    if (this._active && this._graphReady) {
      this._buildRenderer();
    }
  ...
  _ensure() {
    ...
    // real 5.1 has energy in C / SL / SR,
    // up-mixed stereo has none
    const makeAnalyser = () => {
    ...
    // FIX:
    // All Web Audio nodes used by _apply() now exist.
  ...
  _detectLayoutOnce() {
    ...
    for (const a of this._an) {
      ...
      for (let i = 0; i < this._anBuf.length; i++) {
        s += this._anBuf[i] * this._anBuf[i];
      }

      peak = Math.max(
        peak,
        Math.sqrt(s / this._anBuf.length)
      );
    }
  ...
  _buildRenderer() {
    ...
    if (mode === 'multichannel') {
      ...
      dest.channelCount =
        Math.min(6, dest.maxChannelCount);

      dest.channelInterpretation = 'discrete';

      const merger =
        track(ctx.createChannelMerger(6));
      ...
      // no ceiling speakers on a 5.1 set-up:
      // fold heights into the surrounds
      feed(T.TL, 4, 0.5);
      feed(T.TR, 5, 0.5);

      const rs =
        track(ctx.createChannelSplitter(2));
    ...
    if (mode === 'headphones') {
      ...
      Object.entries(HRTF_POSITIONS).forEach(
        ([name, [az, el]]) => {
          const p =
            track(ctx.createPanner());
    ...
    const merge =
      track(ctx.createChannelMerger(2));
    ...
    // widening: each surround also leaks, phase-inverted
    // and slightly delayed, to the far speaker
    const widen = (
      tap,
      dest,
      seconds
  ...
  _setActive(on) {
    // Don't try to manipulate Web Audio nodes until the graph exists.
    if (!this._graphReady) {
      this._active = on;
      return;
    }
    ...
    if (on) {
      ...
      // stops HRTF / reverb CPU cost while bypassed
      this._teardownRenderer();
    }
  }
  ...
  _apply() {
    // FIX:
    // Do not touch any audio nodes until the complete graph exists.
    if (
      !this.ctx ||
      !this._graphReady ||
      !this._active
